...
/**
 * PHA3 ChatGPT visual QA — round 8, final all-in-one visual convergence.
 * Same self-contained git-provenance pattern as rounds 5-7.
 *
 * Round 8's headline change is CONTENT, not geometry: all placeholder/local
 * demo photos were replaced with the 20 ChatGPT-approved Round 8 demo
 * binaries (.webby/client-approved-v2/ROUND8_ASSET_SOURCE.md), SHA256
 * verified 20/20 before use. Because these are genuinely different specific
 * photographs than whatever composited the original master renders, raw
 * pixelmatch diffPixelRatio is EXPECTED to increase across all 10 screens
 * versus round 7, even where the swap is exactly what the asset map called
 * for — this is documented per-screen in the round-8 report rather than
 * misread as a geometry regression.
 *
 * KNOWN, DISCLOSED GAP carried from round 7: masterRegions below are
 * UNCHANGED from round 7 — no new ruler-overlay-precision region
 * measurement was done this round (effort went into asset preflight +
 * mapping + self-QA instead). Still only header-only for /cho-thue/[slug]
 * WEB, /du-an WEB (beyond header+heading+grid+about), /du-an/[slug]
 * WEB+MOBILE.
 */
...
const ROUND7_BASELINE: Record<string, Record<string, number>> = {
  "/": { WEB: 0.449087922318498, MOBILE: 0.4018879155092946 },
  "/cho-thue": { WEB: 0.27235678585159556, MOBILE: 0.4482311284759318 },
  "/cho-thue/[slug]": { WEB: 0.3161867177595394, MOBILE: 0.33458672913932214 },
  "/du-an": { WEB: 0.5204349249017384, MOBILE: 0.5424889095774448 },
  "/du-an/[slug]": { WEB: 0.3715862621178758, MOBILE: 0.3014814362605944 },
};

// Section 13/14-style classification, per screen, updated for round 8.
const DIFF_CLASSIFICATION: Record<string, Record<string, string[]>> = {
  "/": {
    WEB: ["USER_APPROVED_PRODUCT_DELTA", "ASSET_DIFF"],
    MOBILE: ["USER_APPROVED_PRODUCT_DELTA", "ASSET_DIFF"],
  },
  "/cho-thue": {
    // No longer ASSET_BLOCKED_NEEDS_CHATGPT — round 8 resolved this slot
    // with R8_08, an explicit ChatGPT-provided replacement.
    WEB: ["ASSET_DIFF", "DATA_TEXT_DIFF"],
    MOBILE: ["ASSET_DIFF", "GEOMETRY_DIFF", "DATA_TEXT_DIFF"],
  },
  "/cho-thue/[slug]": {
    WEB: ["ASSET_DIFF", "GEOMETRY_DIFF", "DATA_TEXT_DIFF"],
    MOBILE: ["ASSET_DIFF", "GEOMETRY_DIFF", "DATA_TEXT_DIFF", "ANTIALIASING_NOISE"],
  },
  "/du-an": {
...
const OUT_DIR = path.resolve(__dirname, "../../.webby/qa/pha3-v2/round-8");
...
const ROUND7_MASTER_DIR = path.resolve(__dirname, "../../.webby/qa/pha3-v2/round-7/master");
for (const f of fs.readdirSync(ROUND7_MASTER_DIR)) {
  const dest = path.join(MASTER_DIR, f);
  if (!fs.existsSync(dest)) fs.copyFileSync(path.join(ROUND7_MASTER_DIR, f), dest);
}
...
const CAPTURES: CaptureSpec[] = [
  ...
  },
  {
    ...
    masterRegions: {
      WEB: {
        header: SHARED_HEADER_WEB,
        hero: { x: 0, y: 64, width: 935, height: 181 },
        sidebar: { x: 0, y: 275, width: 260, height: 900 },
        list: { x: 280, y: 300, width: 590, height: 1140 },
        pagination: { x: 280, y: 1440, width: 590, height: 50 },
        footer: { x: 0, y: 1500, width: 935, height: 183 },
      },
      MOBILE: {
        header: mobileHeader(),
        heading: { x: 0, y: 95, width: 724, height: 70 },
        "filter-sort": { x: 0, y: 175, width: 724, height: 60 },
        list: { x: 0, y: 260, width: 724, height: 1750 },
        "bottom-nav": { x: 0, y: 2090, width: 724, height: 82 },
      },
    },
  },
  {
...
for (const capture of CAPTURES) {
  for (const [viewportName, vp] of Object.entries(VIEWPORTS) as [keyof typeof VIEWPORTS, (typeof VIEWPORTS)[keyof typeof VIEWPORTS]][]) {
    ...
    test(`${viewportName} ${capture.route}`, async ({ browser }) => {
      ...
      const round7Ratio = ROUND7_BASELINE[capture.route]?.[viewportName] ?? null;
      const absoluteImprovement = diffPixelRatio !== null && round7Ratio !== null ? round7Ratio - diffPixelRatio : null;
      const percentageImprovement =
        absoluteImprovement !== null && round7Ratio !== null && round7Ratio !== 0 ? absoluteImprovement / round7Ratio : null;
      const regressedVsRound7 = absoluteImprovement !== null && absoluteImprovement < 0;
      ...
      results.push({
        ...
        diffPixels,
        diffPixelRatio,
        round7DiffPixelRatio: round7Ratio,
        absoluteImprovement,
      ...
      console.log(
        `${viewportName} ${capture.route} -> round7=${round7Ratio !== null ? (round7Ratio * 100).toFixed(3) + "%" : "n/a"} round8=${diffPixelRatio !== null ? (diffPixelRatio * 100).toFixed(3) + "%" : "n/a"} regressed=${regressedVsRound7}`,
      );
