 ...
 * self-contained git-provenance pattern as rounds 5-6.
 *
 * KNOWN, DISCLOSED GAP (round-7 report, section on masterRegions): section 13
 * of the round-7 spec requires complete masterBox geometry for every visible
 * major region on all strict-PHA1 routes. This round measured/added regions
 * for /cho-thue WEB (hero/sidebar/list/pagination/footer, first-pass visual
 * estimate, not ruler-overlay precision) but did NOT complete equally
 * rigorous region sets for /cho-thue MOBILE (beyond header+filters+list),
 * /cho-thue/[slug] WEB, /du-an WEB (beyond header+heading+grid+about), or
 * /du-an/[slug] WEB+MOBILE beyond header — those still only carry the
 * `header` box carried forward from round 6. This is reported here
 * explicitly rather than filled in with invented numbers.
 */
...
const OUT_DIR = path.resolve(__dirname, "../../.webby/qa/pha3-v2/round-7");
...
const ROUND6_MASTER_DIR = path.resolve(__dirname, "../../.webby/qa/pha3-v2/round-6/master");
for (const f of fs.readdirSync(ROUND6_MASTER_DIR)) {
  const dest = path.join(MASTER_DIR, f);
  if (!fs.existsSync(dest)) fs.copyFileSync(path.join(ROUND6_MASTER_DIR, f), dest);
}
...
const CAPTURES: CaptureSpec[] = [
  ...
  },
  {
    ...
    navigateTo: "/cho-thue",
    masterRegions: {
      // First-pass visual estimate from web-02-chothue.png this round — not
      // yet ruler-overlay precision, disclosed in the round-7 report.
      WEB: {
        header: SHARED_HEADER_WEB,
      ...
      // First-pass visual estimate from mobile-02-chothue.png — filter/sort
      // row + result count + single-column card list + fixed bottom nav.
      MOBILE: {
        header: mobileHeader(),
...
for (const capture of CAPTURES) {
  for (const [viewportName, vp] of Object.entries(VIEWPORTS) as [keyof typeof VIEWPORTS, (typeof VIEWPORTS)[keyof typeof VIEWPORTS]][]) {
    ...
    test(`${viewportName} ${capture.route}`, async ({ browser }) => {
      ...
      const round6Ratio = ROUND6_BASELINE[capture.route]?.[viewportName] ?? null;
      const absoluteImprovement = diffPixelRatio !== null && round6Ratio !== null ? round6Ratio - diffPixelRatio : null;
      const percentageImprovement =
        absoluteImprovement !== null && round6Ratio !== null && round6Ratio !== 0 ? absoluteImprovement / round6Ratio : null;
      const regressedVsRound6 = absoluteImprovement !== null && absoluteImprovement < 0;
      ...
      results.push({
        ...
        diffPixels,
        diffPixelRatio,
        round6DiffPixelRatio: round6Ratio,
        absoluteImprovement,
      ...
      console.log(
        `${viewportName} ${capture.route} -> round6=${round6Ratio !== null ? (round6Ratio * 100).toFixed(3) + "%" : "n/a"} round7=${diffPixelRatio !== null ? (diffPixelRatio * 100).toFixed(3) + "%" : "n/a"} regressed=${regressedVsRound6}`,
      );
