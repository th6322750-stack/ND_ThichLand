 ...
 *
 * Section 1 fix: this spec obtains ALL git provenance itself (git rev-parse
 * / git status --porcelain), directly via child_process, instead of relying
 * on env vars a wrapper script might forget to set (exactly the class of
 * mistake that produced round 4's "gitStatusAfterCapture: unknown"). Ran
 * once before the suite and once in test.afterAll, so both before/after
 * states are real, not guessed.
 */
...
function captureGitState() {
  ...
  // Tracked-source cleanliness only (per round-5 section 1): excludes
  // untracked files/dirs (QA evidence output, .vercel/ build artifacts)
  // from the "is source clean" question, but still reports them
  // separately so nothing is silently hidden.
...
const ROUND4_BASELINE: Record<string, Record<string, number>> = {
  "/": { WEB: 0.2853346, MOBILE: 0.4250862 },
  "/cho-thue": { WEB: 0.2723568, MOBILE: 0.4482311 },
  "/cho-thue/[slug]": { WEB: 0.3235679, MOBILE: 0.333037 },
  "/du-an": { WEB: 0.5158817, MOBILE: 0.5628682 },
  "/du-an/[slug]": { WEB: 0.3715863, MOBILE: 0.3291286 },
};

const OUT_DIR = path.resolve(__dirname, "../../.webby/qa/pha3-v2/round-5");
...
// Masters are the same immutable, byte-identical copies used in every
// prior round — copy once if not already present (never re-encoded).
const ROUND4_MASTER_DIR = path.resolve(__dirname, "../../.webby/qa/pha3-v2/round-4/master");
for (const f of fs.readdirSync(ROUND4_MASTER_DIR)) {
  const dest = path.join(MASTER_DIR, f);
  if (!fs.existsSync(dest)) fs.copyFileSync(path.join(ROUND4_MASTER_DIR, f), dest);
}
...
// Per-route MOBILE header heights — see round-4 section 3 measurement
// (Home fine-measured at 90; the other 4 routes share that as the best
// available estimate — disclosed in the final report, not claimed exact).
    ...
    test(`${viewportName} ${capture.route}`, async ({ browser }) => {
      ...
      const regionBoxes = await page.evaluate(() => {
        ...
        document.querySelectorAll<HTMLElement>("[data-qa-region]").forEach((el) => {
          ...
          // Skip zero-area matches (an off-breakpoint responsive twin of a
          // region name, e.g. a WEB-only block that's `hidden` at MOBILE) —
          // several routes now have 2 DOM nodes sharing one region name,
          // one per breakpoint, and only the visible one should win.
      ...
      const round4Ratio = ROUND4_BASELINE[capture.route]?.[viewportName] ?? null;
      const absoluteImprovement = diffPixelRatio !== null && round4Ratio !== null ? round4Ratio - diffPixelRatio : null;
      const percentageImprovement =
        absoluteImprovement !== null && round4Ratio !== null && round4Ratio !== 0 ? absoluteImprovement / round4Ratio : null;
      const regressedVsRound4 = absoluteImprovement !== null && absoluteImprovement < 0;
      ...
      results.push({
        ...
        diffPixels,
        diffPixelRatio,
        round4DiffPixelRatio: round4Ratio,
        absoluteImprovement,
      ...
      console.log(
        `${viewportName} ${capture.route} -> round4=${round4Ratio !== null ? (round4Ratio * 100).toFixed(3) + "%" : "n/a"} round5=${diffPixelRatio !== null ? (diffPixelRatio * 100).toFixed(3) + "%" : "n/a"} regressed=${regressedVsRound4}`,
      );
